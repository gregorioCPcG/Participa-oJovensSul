#robustez adicional modelo index 3
#Dada a baixa % de partidários, rodamos como robustez o index com as 3 as outras modalidades de participação, os resultados não foram discrepantes em relação ao modelo 5"
# é isso que vamos verificar aqui

# rodar tudo do script_final.R

#e a daí voltar aqui


#antes de seguir fazer esses ajustes
rm(list = setdiff(ls(), "modelo_Particip6index"))#mantedo o modelo com index4



# aí rodar parte do código abaixo do script final de novo para a robustez adicional

#replicando parte dos códigos ########
...
# alteração da robustez  ####

#criar index participação com 3
df <- subset(df, select=-c(Partisan))
str(df[,278:280])#verificar se contém as 3 agora
imp <- mice(df[,278:280], seed=23109)#a seed sempre essa 23109
df[,278:280] <- complete(imp, 1)#sempre a m como  destino da mputação , empre escolher a 1
rm(imp)
summary(df[,278:280])#pouca alteração

df$IndexParticipacao_com3 <- df$OnLine + df$Protest + df$Votaria
hist(df$IndexParticipacao_com3,breaks=50)#Ok
df$religiosidade_numeric <- as.numeric(df$religiosidade_bin)
df$etniaBranco_numeric <- as.numeric(df$Etnia_Branco)
summary(df[,267:284])#usar para tabela 1

sd(df$escolaridadePais)
sd(df$IndexParticipacao)


# modelos index3 e index4 comparados
modelo_Particip6index -> modelo_index4

modelo_index3 <- lm(IndexParticipacao_com3 ~ Genero+Tipo_Escola+Classe+
                              religiosidade_bin+Etnia_Branco+
                              escolaridadePais+Intolerancia+Privatista,
                            data=df)
tab_model(modelo_index3, modelo_index4)
