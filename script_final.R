...
sum(is.na(df$OnLine))
imp <- mice(df[,277:280], seed=23109)#a seed sempre essa 23109
df[,277:280] <- complete(imp, 1)#sempre a m como  destino da mputação , empre escolher a 1
rm(imp)
...
c <- plot_cap(modelo_Particip6index, condition=c("Genero"), conf_level = .9)
c + theme_bw()
